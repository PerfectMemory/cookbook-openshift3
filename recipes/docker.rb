...
if is_node_server || node['cookbook-openshift3']['deploy_containerized']
  yum_package 'docker' do
    action :install
    version node['cookbook-openshift3']['upgrade'] ? (helper.get_nodevar('upgrade_docker_version') unless helper.get_nodevar('upgrade_docker_version').nil?) : (helper.get_nodevar('docker_version') unless helper.get_nodevar('docker_version').nil?)
    retries 3
    ...
    only_if do
      ::Mixlib::ShellOut.new('rpm -q docker').run_command.error? || node['cookbook-openshift3']['upgrade'] || ::File.file?(node['cookbook-openshift3']['adhoc_reset_control_flag'])
    end
  end
