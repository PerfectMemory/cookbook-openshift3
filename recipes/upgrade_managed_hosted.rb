...
template "#{Chef::Config[:file_cache_path]}/router-patch" do
  ...
  variables(
    lazy do
      {
        router_image: Mixlib::ShellOut.new("#{node['cookbook-openshift3']['openshift_common_client_binary']} get dc/router -n #{node['cookbook-openshift3']['openshift_hosted_router_namespace']} -o jsonpath='{.spec.template.spec.containers[0].image}' --config=#{node['cookbook-openshift3']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['cookbook-openshift3']['openshift_master_loopback_api_url']}").run_command.stdout.strip.gsub(/:v.+/, ":#{hosted_upgrade_version}")
      }
    end
  )
end
