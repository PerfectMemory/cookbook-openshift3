  ...
  if is_remove_etcd_server
    ...
    unless ::File.file?("#{node['cookbook-openshift3']['etcd_data_dir']}/.removed")
      execute 'Check member has been removed from cluster' do
        command "[[ $(/usr/bin/etcdctl --cert-file #{node['cookbook-openshift3']['etcd_peer_file']} --key-file #{node['cookbook-openshift3']['etcd_peer_key']} --ca-file #{node['cookbook-openshift3']['etcd_ca_cert']} -C https://#{first_etcd['ipaddress']}:2379 member list | grep -c #{node['fqdn']}) -eq 0 ]]"
        retries 60
        retry_delay 5
