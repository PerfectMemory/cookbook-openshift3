...
action :create_ng do
  ...
  converge_by 'Create Master configuration file' do
    ...
    admission_default = node['cookbook-openshift3']['openshift_master_admission_plugin_config'].empty? ? { 'openshift.io/ImagePolicy' => { 'configuration' => { 'apiVersion' => 'v1', 'executionRules' => [{ 'matchImageAnnotations' => [{ 'key' => 'images.openshift.io/deny-execution', 'value' => 'true' }], 'name' => 'execution-denied', 'onResources' => [{ 'resource' => 'pods' }, { 'resource' => 'builds' }], 'reject' => true, 'skipOnResolutionFailure' => true }], 'kind' => 'ImagePolicyConfig' } } } : node['cookbook-openshift3']['openshift_master_admission_plugin_config']
    ...
    ruby_block 'Prepare YAML' do
      block do
        ...
        file new_resource.master_file do
          content pre_master.to_yaml
          unless node['chef_packages']['chef']['version'] == node['cookbook-openshift3']['switch_off_provider_notify_version']
            notifies :run, 'execute[Restart API]', :immediately unless node['cookbook-openshift3']['upgrade']
            notifies :run, 'execute[Restart Controller]', :immediately unless node['cookbook-openshift3']['upgrade']
          end
        end
      end
    end
