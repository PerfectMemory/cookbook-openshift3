...
def generate_secrets(secret)
  ...
  ::File.new("#{FOLDER}/templates/#{secret['metadata']['name']}.yaml", 'w') { |f| f << secret_skel.to_yaml }
end
...
def generate_routes(route)
  ...
  ::File.new("#{FOLDER}/templates/#{route['metadata']['name']}-route.yaml", 'w') { |f| f << route_skel.to_yaml }
end
...
def generate_serviceaccounts(serviceaccount)
  ...
  ::File.new("#{FOLDER}/templates/#{serviceaccount['metadata']['name']}-serviceaccount.yaml", 'w') { |f| f << serviceaccount_skel.to_yaml }
end
...
def generate_rolebindings(rolebinding)
  ...
  ::File.new("#{FOLDER}/templates/#{rolebinding['metadata']['name']}-rolebinding.yaml", 'w') { |f| f << rolebinding_skel.to_yaml }
end
...
def generate_roles(role)
  ...
  ::File.new("#{FOLDER}/templates/#{role['metadata']['name']}-role.yaml", 'w') { |f| f << role_skel.to_yaml }
end
...
def generate_services(service)
  ...
  ::File.new("#{FOLDER}/templates/#{service['metadata']['name']}-service.yaml", 'w') { |f| f << service_skel.to_yaml }
end

action :delete do
  converge_by "Uninstalling Metrics on #{node['fqdn']}" do
    CERT_FOLDER = node['cookbook-openshift3']['openshift_common_base_dir'] + '/metrics'
    ...
    file node['cookbook-openshift3']['openshift_hosted_metrics_flag'] do
      action :delete
    end
  end
end
...
action :create do
  converge_by "Deploying Metrics on #{node['fqdn']}" do
    ...
    ruby_block 'Create Routes' do
      block do
        [{ 'metadata' => { 'name' => 'hawkular-metrics', 'labels' => { 'metrics-infra' => 'hawkular-metrics' } }, 'spec' => { 'host' => node['cookbook-openshift3']['openshift_metrics_hawkular_hostname'], 'to' => { 'kind' => 'Service', 'name' => 'hawkular-metrics' }, 'tls' => { 'termination' => 'reencrypt', 'destinationCACertificate' => ::File.read("#{CERT_FOLDER}/ca.crt") } } }].each do |route|
    ...
    template 'Generate cassandra replication controller' do
      ...
      variables(
        ose_major_version: ose_major_version,
        fsgroup: Mixlib::ShellOut.new("#{node['cookbook-openshift3']['openshift_common_client_binary']} get namespace #{node['cookbook-openshift3']['openshift_metrics_project']} -o 'go-template={{range $key,$value := .metadata.annotations}}{{$key}}: {{$value}}{{\"\\n\"}}{{end}}' --config=#{node['cookbook-openshift3']['openshift_master_config_dir']}/admin.kubeconfig | awk '/sa.scc.supplemental-groups/ {split($2,a,\"/\"); print a[1]}'").run_command.stdout.strip,
        selinux_level: Mixlib::ShellOut.new("#{node['cookbook-openshift3']['openshift_common_client_binary']} get namespace #{node['cookbook-openshift3']['openshift_metrics_project']} -o 'go-template={{range $key,$value := .metadata.annotations}}{{$key}}: {{$value}}{{\"\\n\"}}{{end}}' --config=#{node['cookbook-openshift3']['openshift_master_config_dir']}/admin.kubeconfig | awk '/sa.scc.mcs/ { print $2}'").run_command.stdout.strip,
        run_as_uid: Mixlib::ShellOut.new("#{node['cookbook-openshift3']['openshift_common_client_binary']} get namespace #{node['cookbook-openshift3']['openshift_metrics_project']} -o 'go-template={{range $key,$value := .metadata.annotations}}{{$key}}: {{$value}}{{\"\\n\"}}{{end}}' --config=#{node['cookbook-openshift3']['openshift_master_config_dir']}/admin.kubeconfig | awk '/sa.scc.uid-range/ {split($2,a,\"/\"); print a[1]}'").run_command.stdout.strip
      )
      sensitive true
    end
    ...
    unless ::File.file?(node['cookbook-openshift3']['openshift_hosted_metrics_flag'])
